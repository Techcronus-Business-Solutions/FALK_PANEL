...
{
    public class PricingMasterExteriorPlugin : PluginBase
    {
        public PricingMasterExteriorPlugin() : base(typeof(PricingMasterExteriorPlugin)) { }
        ...
        protected override void ExecuteCrmPlugin(LocalPluginContext localcontext)
        {
            ...
            try
            {
                if (context.InputParameters.Contains(CONST_TARGETENTITY) && context.InputParameters[CONST_TARGETENTITY] is Entity)
                {
                    ...
                    if (targetEntity.LogicalName == "tbs_pricingmasterexterior")
                    {
                        // ---------------- CREATE (PreOperation) ----------------
            ...
            {
                tracingService.Trace("PricingMasterExteriorPlugin Exception: {0}", ex.ToString());
                throw new InvalidPluginExecutionException($"Error in PricingMasterExteriorPlugin: {ex.Message}");
            }
        }
