...
{
    public class QuoteExtraPricing : PluginBase
    {
        public QuoteExtraPricing() : base(typeof(QuoteExtraPricing)) { }
        ...
        protected override void ExecuteCrmPlugin(LocalPluginContext localcontext)
        {
            ...
            try
            {
                        ...
                        if (context.MessageName == CONST_UPDATE && context.Stage == PostOperation)
                        {
                            try
                            {
                                if (targetEntity.Contains("quantity"))
                                {
                                    ...
                                    Guid quoteId = PostImage.GetAttributeValue<EntityReference>("quoteid").Id;

                                    Entity Quote = service.Retrieve("quote", quoteId, new ColumnSet("skippricecalculation"));
                                    int? skipPriceCalculation = GetOptionSetAttributeValue(Quote, "skippricecalculation");

                                    if (skipPriceCalculation.HasValue && skipPriceCalculation.Value == 1)
                                    {
                                        tracingService.Trace("Converted Quote");
                                        return;
                                    }

                                    int totalSQFT = GetTotalSQFTFromQuoteProducts(quoteId);
                                    int totalFilteredSQFT = GetFilteredSQFTFromQuoteProducts(quoteId);

                                    Entity updateQuote = new Entity("quote");
                                    updateQuote.Id = quoteId;
                                    updateQuote["tbs_totalsqft"] = totalSQFT;
                                    updateQuote["tbs_totalfilteredsqft"] = totalFilteredSQFT;
                    ...
                    if (targetEntity.LogicalName == "quote")
                    {
                        ...
                        if (context.MessageName == CONST_UPDATE && context.Stage == PostOperation)
                        {
                            if (targetEntity.Contains("tbs_shopdrawingsprice") || targetEntity.Contains("tbs_stampedandsealedshopdrawingsprice") || targetEntity.Contains("tbs_roofwarrantyprice") || targetEntity.Contains("tbs_factorymutualspecifiedrequirementsprice") || targetEntity.Contains("tbs_buyamericanprice"))
                            {
                                ...
                                Entity updateQuote = new Entity("quote", targetEntity.Id);
                                updateQuote["freightamount"] = new Money(totalExtraPricing);
                                service.Update(updateQuote);
                            }
                        }
                    }
                ...
                else if (context.InputParameters.Contains(CONST_TARGETENTITY) && (context.InputParameters[CONST_TARGETENTITY] is EntityReference))
                {
                    ...
                    if (targetEntityRef.LogicalName == "quotedetail")
                    {
                        if (context.MessageName == CONST_DELETE && context.Stage == PostOperation)
                        {
                            ...
                            {
                                Entity PreImage = context.PreEntityImages["PreImage"];
                                Guid quoteId = PreImage.GetAttributeValue<EntityReference>("quoteid").Id;
        ...
        private int GetTotalSQFTFromQuoteProducts(Guid quoteId)
        {
            int totalSQFT = 0;
        ...
        private int GetFilteredSQFTFromQuoteProducts(Guid quoteId)
        {
            // 2, 2.5, 3, 4, 5, 6, 8
