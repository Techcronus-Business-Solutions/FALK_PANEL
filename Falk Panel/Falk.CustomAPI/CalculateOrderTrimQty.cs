...
{
    public class CalculateOrderTrimQty : CustomAPIBase
    {
        public CalculateOrderTrimQty() : base(typeof(CalculateOrderTrimQty)) { }
        ...
        protected override void ExecuteCrmPlugin(LocalPluginContext localcontext)
        {
            ...
            try
            {
                ...
                Console.WriteLine("Order ProductId = " + orderProductId.ToString());

                Entity orderProduct = GetOrderProduct(orderProductId);

                EntityCollection trims = GetTrims(orderProductId);

                CalculationContext calccontext = BuildCalculationContext(orderProduct);
        ...
        private void CalculateAllTrims(EntityCollection trims, CalculationContext context)
        {
            try
            {
                ...
                foreach (var config in configs.Values)
                {
                    if (config != null)
                    {
                        tracingService.Trace("panel trim = " + config.OrderTrimId);
        ...
        private void CalculateTrim(TrimConfiguration config, Dictionary<Guid, TrimConfiguration> configs, CalculationContext context)
        {
            try
            {
                ...
                if (qty.HasValue)
                {
                    ...
                    tracingService.Trace(config.OrderTrimId.ToString());

                    int currentQty = config.Trim.GetAttributeValue<int?>("tbs_quantity") ?? 0;

                    if (currentQty != qty.Value)
                    {
                        UpdateTrimQuantity(config.OrderTrimId, qty.Value);
                    }
                }
                else
                {
        ...
        private CalculationContext BuildCalculationContext(Entity orderProduct)
        {
            return new CalculationContext
            {
                SqFt = orderProduct.GetAttributeValue<decimal>("quantity"),

                LinearFeet = orderProduct.GetAttributeValue<decimal>("tbs_linearfeet"),

                OrderProdId = orderProduct.ToEntityReference()
            };
        }
        ...
        public class TrimConfiguration
        {
            ...
            public Guid OrderTrimId { get; set; }
